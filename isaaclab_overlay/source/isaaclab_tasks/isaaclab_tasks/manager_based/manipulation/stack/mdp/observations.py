...
def object_placed(
    ...
    """Check if an object has been placed onto a (fixed) container and released.

    Unlike :func:`object_stacked`, the lower target is a static container (e.g. a colored
    platform) rather than another cube. The object is considered placed when its XY position
    is close to the container center, it rests near the container surface, and the gripper is
    open (i.e. the cube has been released).
    """
    ...
    pos_diff = obj.data.root_pos_w - container.data.root_pos_w
    xy_dist = torch.linalg.vector_norm(pos_diff[:, :2], dim=1)
    height_dist = torch.abs(pos_diff[:, 2])

    placed = torch.logical_and(xy_dist < xy_threshold, height_dist < max_height)
